...
print ("")
print ("须知：")
print ("* 设置重定向，意味着当来访的请求不符合 ShadowSocksR 数据包类型的时候，ShadowSocksR 会自动将该请求重定向到本机的或其他域的空间")
print ("* 典型的配置，是将外界访问本机 80、443 端口且不符合以上规则的请求，全部重定向到本机的某个端口，然后由 Apache、Nginx 或 Caddy 来监听该端口，实现 ShadowSocksR 在正常网站背后的隐藏或反代功能")
...
print ("* 设置好重定向功能后，ShadowSocksR 的主通信端口会被还原为 443，若曾用过本脚本设置重定向，取消配置后主端口仍为 443")
print ("")
print ("当前重定向参数为：")
ShowRedirect()
print ("")
